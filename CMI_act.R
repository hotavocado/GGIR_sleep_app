...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      # Input: Select file
      fileInput("file1", "Choose basic .RData file",
                multiple = FALSE),
      fileInput("file2", "Choose ms4.out .RData file",
                multiple = FALSE),
...
server <- function(input, output, session) {
  ...
  output$MyPlot <- renderPlot({
    ...
    #load('/Users/nathalia.esper/Documents/Actigraph/output_csv/meta/ms4.out/NDARAA948VFH.csv.RData')
    
    createAlert(session, 'upload_complete', title = 'Data Import Complete',
                content = 'Wait until analysis is complete',
    ...
    if (length(nightsi) > 0) {
      ...
      for (g in 1:3) {
        ...
        #ggplot(x, aes(x, acc)) + geom_points()
  ...
  output$info <- renderText({
    xy_range_str <- function(e) {
      ...
      #sleep_temp = (round(e$xmin,1)*24)/8640
      #wake_temp = (round(e$xmax, 1)*24)/8640
      #paste0("sleep = ", sleep_temp, "\nwake = ", wake_temp)
      paste0("sleep = ", round(e$xmin,1), "\nwake = ", round(e$xmax,1))
    }
